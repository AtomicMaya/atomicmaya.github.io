...
import img_description from '../../../assets/images/post/2021/2021-10-11_ctf/c4/description.png'
...
- `Φ` is a string. Specifically it is the string <code>{'"UvSgI5/_M26b<z]Ks+e)rn!l}1=p 0Z$`3A.kf^jD>a{yBC(4Y|9;c-G%N?8xho,7:t[iWmuF&"d*LE"'}</code>. Like with any other array-like type in JS, you can then use `Φ[i]` to access the character at index `i`.
...
- `_`: The 192-bit AES key, which -- unhelpfully -- spells out <code>{`¬FLAG{TH1$_1$_B1T_T4NG0}`}</code> (a false flag, <em>because of course</em>).
...
- `xr`: Performs a linear XOR on a set of lists.<br/>`xr([1,2,3,4], [9,8,7,6]) => [1^9, 2^8, 3^7, 4^6]`
...
- `zip`: Zips two lists together based on index.<br/>`zip([1,2,3,4], [9,8,7,6]) => [[1,9], [2,8], [3,7], [4,6]]`
